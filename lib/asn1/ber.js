...
function decodeTag(s, tag) {
	...
	//log.debug('BER decode Tag : Class Format = ' + ((nextTag & 0xE0) == (tag.tagClass | tag.tagFormat)) + ', number = ' + nextTagNumber + ' <> ' + tag.tagNumber);
...
function decodeLength(s) {
	...
	if(size & 0x80) {
		...
		if(size == 1) {
			...
			throw new Error('NODE_RDP_ASN1_BER_INVALID_LENGTH');
		}
	}
	return size;
};
...
function decode(s, tag) {
	if (!decodeTag(s, tag)) {
		throw new Error('NODE_RDP_ASN1_BER_INVALID_TAG');
	}
...
function encodeTag(tag) {
	if(tag.tagNumber > 30) {
		return new type.Component([new type.UInt8(tag.tagClass | tag.tagFormat), new type.UInt8(tag.tagNumber)]);
	}
	else {
