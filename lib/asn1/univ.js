...
Integer.prototype.decode = function(s, decoder) {
	var integerStream = new type.Stream(decoder.decode(s, this.tag).value);
	for (var i = 0; i < integerStream.availableLength(); i++) {
...
Integer.prototype.encode = function(encoder) {
	if(this.value <= 0xff) {
        return encoder.encode(new type.UInt8(value));
    }
    else if(this.value <= 0xffff) {
        return encoder.encode(new type.UInt16Be(value));
    }
    else {
        return encoder.encode(new type.UInt32Be(value));
    }
};
...
Sequence.prototype.decode = function(s, decoder) {
	...
	for (var i in this.value) {
		...
		try {
			...
			if (!this.value[i].opt) {
				throw 'NODE_RDP_ASN1_UNIV_SEQUENCE_FIELD_NOT_PRESENT';
			}
...
BitString.prototype.toBuffer = function () {
	var resultStream = new type.Stream(this.value.length / 8);
	for (var i = 0; i < this.value.length; i += 8) {
		var currentOctet = 0;
		for (var j = 0; j < 8; j++) {
