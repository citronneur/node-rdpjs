...
Type.prototype.read = function(s) {
	...
	if(this.opt.constant) {
		...
		if(oldValue != this.value) {
			...
			throw new Error("NODE_RDP_CORE_TYPE_CONSTANT_VALUE_MISMATCH");
		}
	}
	else {
		this.readValue(s);
...
Component.prototype.readValue = function(s) {
	...
	for(var i in this.obj) {
		...
		if (this.opt.readLength) {
			var readLength = s.offset - start;
			if (readLength > this.opt.readLength.value) {
				if(this.obj[i].opt.optional) {
					s.offset -= this.obj[i].size()
				}
				else {
					throw new Error('NODE_RDP_CORE_TYPE_UNABLE_TO_READ_NON_OPTIONAL_FIELD');
				}
			}
		}
	}
	...
	// padding
	if (this.opt.readLength) {
		var deltaLength = s.offset - (start + this.opt.readLength.value);
		if (deltaLength > 0) {
			log.warn('still have available data : read it as padding');
			s.offset += deltaLength;
		}
	}
};
...
Factory.prototype._size_ = function() {
	throw new Error('NODE_RDP_CORE_TYPE_FACTORY_TYPE_HAVE_NO_SIZE');
};
