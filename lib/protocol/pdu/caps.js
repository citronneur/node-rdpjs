...
function orderCapability(orders, opt) {
	if(orders && orders.size() != 32) {
		throw new Error('NODE_RDP_PROTOCOL_PDU_CAPS_BAD_ORDERS_SIZE');
	}
	...
	var self = {
        ...
        orderSupport : orders || new type.Factory(function(s) {
        	self.orderSupport = new BinaryString(null, {readLegth : new type.CallableValue(32)}).read(s);
        }),
...
function glyphCapability(entries, opt) {
	var self = {
        ...
        glyphSupportLevel : UInt16Le(GlyphSupport.GLYPH_SUPPORT_NONE),
        pad2octets : new type.UInt16Le()
	};
	...
	var self = {
		...
		capability : cap || new type.Factory(function(s) {
			...
			case CapsType.CAPSTYPE_ORDER:
				self.capability = orderCapability({readLength : new type.CallableValue(function() {
					return self.lengthCapability.value - 4;
				})}).read(s);
				break;
			...
			case CapsType.CAPSETTYPE_MULTIFRAGMENTUPDATE:
				...
				break;
			default:
				log.info('unknown capability ' + self.capabilitySetType.value);
