...
function scope() {
	var self = {
		scope : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_SCOPE_BLOB)
	};
...
function serverLicenseRequest(opt) {
	var self = {
		__TYPE__ : MESSAGE_TYPE.LICENSE_REQUEST,
		serverRandom : new type.BinaryString(new Buffer(Array(32 + 1).join('\x00')), { readLength : new type.CallableValue(32) } ),
        productInfo : productInformation(),
        keyExchangeList : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_KEY_EXCHG_ALG_BLOB),
        serverCertificate : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_CERTIFICATE_BLOB),
        scopeList : scopeList()
	};
...
function clientNewLicenseRequest(opt) {
	var self = {
		__TYPE__ : MESSAGE_TYPE.NEW_LICENSE_REQUEST,
        ...
        clientRandom : new type.BinaryString(Array(32 + 1).join('\x00'), { readLength : new type.CallableValue(32) }),
        encryptedPreMasterSecret : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_RANDOM_BLOB),
        ClientUserName : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_CLIENT_USER_NAME_BLOB),
        ClientMachineName : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_CLIENT_MACHINE_NAME_BLOB)
	};
...
function serverPlatformChallenge(opt) {
	var self = {
		__TYPE__ : MESSAGE_TYPE.PLATFORM_CHALLENGE,
		connectFlags : new type.UInt32Le(),
        encryptedPlatformChallenge : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_ANY_BLOB),
        MACData : new BinaryString(new Buffer(Array(16 + 1).join('\x00')), { readLength : new type.CallableValue(16) })
	};
...
function clientPLatformChallengeResponse(opt) {
	var self = {
		__TYPE__ : MESSAGE_TYPE.PLATFORM_CHALLENGE_RESPONSE,
		encryptedPlatformChallengeResponse : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_DATA_BLOB),
        encryptedHWID : licenseBinaryBlob(BINARY_BLOB_TYPE.BB_DATA_BLOB),
        MACData : new BinaryString(new Buffer(Array(16 + 1).join('\x00')), { readLength : new type.CallableValue(16) })
	};
...
function licensePacket() {
	var self = {
		bMsgtype : new type.UInt8(function() {
			return self.licensingMessage.__TYPE__;
		}),
