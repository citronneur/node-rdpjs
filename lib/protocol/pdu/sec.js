...
Sec.prototype.send = function(message) {
	if (this.enableEncryption) {
		throw new Error('NODE_RDP_PROTOCOL_PDU_SEC_ENCRYPT_NOT_IMPLEMENTED');
	}
 ...
 * Connect event
 */
Client.prototype.connect = function(gccClient, gccServer, userId) {
...
Client.prototype.sendClientChallengeResponse = function(platformChallenge) {
	...
	if (serverChallenge.toString('ucs2') != 'TEST\x00') {
		throw new Error('NODE_RDP_PROTOCOL_PDU_SEC_INVALID_LICENSE_CHALLENGE');
	}
...
Sec.prototype.recvLicense = function(s) {
    ...
    if (message.bMsgtype.value == lic.MessageType.NEW_LICENSE || 
    	...
    	this.emit('connect', this.gccClient.core, this.userId);
    	var self = this;
