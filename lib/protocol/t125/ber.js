 ...
 * @returns {object} BER encoded length
 */
 ...
 * @returns {object} BER encoded application tag
 */
function writeApplicationTag(tag, size) {
    if(tag > 30) {
        return new type.Component([UInt8((BER_CLASS.BER_CLASS_APPL | BER_PC.BER_CONSTRUCT) | BER_TAG.BER_TAG_MASK), new type.UInt8(tag), writeLength(size)]);
    }
    else {
        return new type.Component([UInt8((BER_CLASS.BER_CLASS_APPL | BER_PC.BER_CONSTRUCT) | (BER_TAG.BER_TAG_MASK & tag)), writeLength(size)]);
    }
}
...
/**
 * @param b {boolean}
 * @returns {object} BER encoded boolean
 */
function writeBoolean(b) {
    return new type.Component([writeUniversalTag(BER_TAG.BER_TAG_BOOLEAN, false), writeLength(1), UInt8(b ? 0xff : 0x00)]);
}
...
/**
 * @param value {integer}
 * @returns {object} BER encoded integer value
 */
