...
function clientCoreData(opt) {
	var self = {
		...
		clientDigProductId : new type.BinaryString(new Buffer(Array(64 + 1).join('\x00')), readLength = new type.CallableValue(64), { optional : true }),
...
function readConferenceCreateResponse(s) {
	...
	if(!per.readObjectIdentifier(s, t124_02_98_oid)) {
		throw new Error('NODE_RDP_PROTOCOL_T125_GCC_BAD_OBJECT_IDENTIFIER_T124');
	}
	...
	if (!per.readOctetStream(s, h221_sc_key, 4)) {
		throw new Error('NODE_RDP_PROTOCOL_T125_GCC_BAD_H221_SC_KEY');
	}
