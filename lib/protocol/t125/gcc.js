	...
	var self = {
	    ...
	    data : data || new type.Factory(function(s){
	    	...
	    	default:
	    		...
	    		self.data = new type.BinaryString(null, { readLen : new type.CallableValue(self.length.value - 4) }).read(s);
	    	}
	    })
	};
...
function clientCoreData() {
	var self = {
		...
		clientName : new type.BinaryString(new Buffer("speyrefitte-Vir\x00", 'ucs2'), { readLen : new type.CallableValue(32) }),
		...
		imeFileName : new type.BinaryString(new Buffer(Array(64 + 1).join("\x00")), { readLen : new type.CallableValue(64), optional : true }),
		...
		clientDigProductId : new type.BinaryString(new Buffer(Array(64 + 1).join("\x00")), readLen = new type.CallableValue(64), { optional : true }),
...
function clientSettings() {
	return settings(new type.Component([gccBlock(clientCoreData()), gccBlock(clientNetworkData()), gccBlock(clientSecurityData())]));
}
