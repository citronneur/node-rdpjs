...
function GCCBlock(data) {
	var self = {
	    ...
	    // data block
	    data : data || type.Factory(function(){
	    	switch(self.type.value) {
	    	case MESSAGE_TYPE.SC_CORE:
	    		self.data = ClientCoreData()
	    		break;
	    	...
	    	default:
	    		...
	    		self.data = new type.String(null, { readLen : new type.CallableValue(self.length.value - 4) });
	    	}
	    })
	};
...
function ClientCoreData() {
	var self = {
		...
		clientName : new type.String("rdpy" + "\x00"*11, { readLen : new type.CallableValue(32), unicode : true }),
		...
		earlyCapabilityFlags : new type.UInt16Le(CAPABILITY_FLAGS.RNS_UD_CS_SUPPORT_ERRINFO_PDU, { optional : true }),
		clientDigProductId : new type.String(Array(64 + 1).join("\x00"), readLen = CallableValue(64), { optional : true }),
		connectionType : new type.UInt8({ optional : true }),
		pad1octet : new type.UInt8({ optional : true }),
		serverSelectedProtocol : new type.UInt32Le({ optional : true })
	};
	return new type.Component(self);
}
