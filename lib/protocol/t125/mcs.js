...
MCS.prototype.recv = function(s) {
	...
	if (readMCSPDUHeader(opcode, DomainMCSPDU.DISCONNECT_PROVIDER_ULTIMATUM)) {
		...
		this.emit('error', new Error('NODE_RDP_PROTOCOL_T125_MCS_BAD_RECEIVE_OPCODE'));
		return;
	}
...
Client.prototype.recvAttachUserConfirm = function(s) {
    if (!readMCSPDUHeader(new type.UInt8().read(s).value, DomainMCSPDU.ATTACH_USER_CONFIRM)) {
    	this.emit('error', new Error('NODE_RDP_PROTOCOL_T125_MCS_BAD_HEADER'));
    	return;
    }
    
    if (per.readEnumerates(s) !== 0) {
    	this.emit('error', new Error('NODE_RDP_PROTOCOL_T125_MCS_SERVER_REJECT_USER'));
    	return;
    }
...
Client.prototype.recvChannelJoinConfirm = function(s) {
    ...
    if (!readMCSPDUHeader(opcode, DomainMCSPDU.CHANNEL_JOIN_CONFIRM)) {
    	this.emit('error', new Error('NODE_RDP_PROTOCOL_T125_MCS_WAIT_CHANNEL_JOIN_CONFIRM'));
    	return;
    }
    ...
    if (this.userId !== userId) {
    	this.emit('error', new Error('NODE_RDP_PROTOCOL_T125_MCS_INVALID_USER_ID'));
    	return;
    }
    ...
    if ((confirm !== 0) && (channelId === Channel.MCS_GLOBAL_CHANNEL || channelId === this.userId)) {
    	this.emit('error', new Error('NODE_RDP_PROTOCOL_T125_MCS_SERVER_MUST_CONFIRM_STATIC_CHANNEL'));
    	return;
    }
