...
function writeNumericString(nStr, minValue) {
    ...
    for(var i = 0; i < length; i += 2) {
        var c1 = String.charCodeAt(nStr[i]);
    	var c2 = 0;
        if(i + 1 < length) {
            c2 = String.charCodeAt(nStr[i + 1]);
        }
        else {
            c2 = 0x30;
        }
...
/**
 * @param length {integer} length of padding
 * @returns {type.String} per encoded padding
 */
function writePadding(length) {
    return new type.String(Array(length).join("\x00"));
}
...
function readOctetStream(s, octetStream, minValue) {
    ...
    for(var i = 0; i < size; i++) {
        ...
        if(String.charCodeAt(octetStream[i]) != c.value) {
            return false;
        }
    }
...
function writeOctetStream(oStr, minValue) {
    ...
    for(var i = 0; i < length; i++) {
        result[result.length] = new typeUInt8(String.charCodeAt(oStr[i]));
    }
