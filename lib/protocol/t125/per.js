...
function readInteger16(s, minimum) {
    var result = new type.UInt16Be();
    s.readValue(result);
    return result.value + (minimum || 0);
}
...
function readObjectIdentifier(s, oid) {
    var size = readLength(s);
    if(size != 5) {
    	throw "NODE_RDP_PROTOCOL_T125_PER_BAD_OID_LENGTH";
    }
    ...
    for(var i in oid) {
    	if(oid[i] != a_oid[i]) throw "NODE_RDP_PROTOCOL_T125_PER_BAD_OID";
    }
}
