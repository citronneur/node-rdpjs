...
Client.prototype.recvConnectionConfirm = function(data) {
	...
	if(message.protocolNeg.obj.type == NEGOTIATION_TYPE.TYPE_RDP_NEG_FAILURE) {
		throw "RDP_NEG_FAILURE";
	}
	
	if(message.protocolNeg.obj.type == NEGOTIATION_TYPE.TYPE_RDP_NEG_RSP) {
		this.selectedProtocol = message.protocolNeg.obj.result.value;
	}
	
	if(this.selectedProtocol in [PROTOCOLS.PROTOCOL_HYBRID, PROTOCOLS.PROTOCOL_HYBRID_EX]) {
		throw "NLA_NOT_SUPPORTED";
	}
	
	if(this.selectedProtocol == PROTOCOLS.PROTOCOL_SSL) {
		this.transport.startTLS(function(err) {
